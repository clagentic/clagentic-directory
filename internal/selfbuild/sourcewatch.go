...
// SourceWatchConfig configures the source-watch mechanism.
type SourceWatchConfig struct {
	// MemoryAPIURL is the memory API base URL, e.g. "http://localhost:9100".
	MemoryAPIURL string
	// BaseDir is the root directory where proposed_changes/ will be written.
	BaseDir string
	// PollInterval controls how often the file-diff stream is checked. Default 60s.
	...
	// HTTPTimeout for memory API calls.
	HTTPTimeout time.Duration
}

// SourceWatcher polls the configured memory API stream for agent-definition-file
...
// SourceEvent is a single event from the memory API file-diff stream.
...
// NewSourceWatcher creates a SourceWatcher with the given config.
func NewSourceWatcher(cfg SourceWatchConfig) *SourceWatcher {
...
// Run polls the memory API file-diff stream until ctx is cancelled.
func (w *SourceWatcher) Run(ctx context.Context) {
	slog.Info("source-watch: starting", "url", w.cfg.MemoryAPIURL, "interval", w.cfg.PollInterval)
	...
	for {
		select {
		case <-ctx.Done():
			slog.Info("source-watch: stopped")
			return
		case <-ticker.C:
			if err := w.poll(ctx); err != nil {
				slog.Warn("source-watch: poll error", "err", err)
			}
		}
	}
}

// ProcessEvents processes a slice of file-diff events (exposed for testing).
func (w *SourceWatcher) ProcessEvents(events []SourceEvent) ([]string, error) {
	var written []string
	for _, ev := range events {
		...
		if w.isDupe(agentName) {
			slog.Debug("source-watch: skipping duplicate", "agent", agentName)
			continue
		}
		...
		notes := []string{
			...
			fmt.Sprintf("Source event ID: %s", ev.ID),
		}
		if declared {
		...
		if err != nil {
			slog.Warn("source-watch: write error", "agent", agentName, "err", err)
			continue
		}
		w.markSeen(agentName)
		slog.Info("source-watch: proposed change written", "agent", agentName, "path", path)
		written = append(written, path)
	}
	return written, nil
}

func (w *SourceWatcher) poll(ctx context.Context) error {
	url := fmt.Sprintf("%s/v1/events?kinds=file-diff", w.cfg.MemoryAPIURL)
	...
	if err := json.NewDecoder(resp.Body).Decode(&events); err != nil {
		return fmt.Errorf("decode source events: %w", err)
	}
...
func (w *SourceWatcher) isDupe(agentName string) bool {
	w.mu.Lock()
	defer w.mu.Unlock()
...
func (w *SourceWatcher) markSeen(agentName string) {
	w.mu.Lock()
	defer w.mu.Unlock()
